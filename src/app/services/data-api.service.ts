...
export class DataApiService {
  ...
  constructor(
	...
	getPatientMessages(id: string){
		const url_api = ` http://192.168.0.130:3032/api/messages?filter[where][idpatient]=${id}`;
		// return this.http.get(url_api); http://192.168.0.130:3032/
		return (this.messages = this.http.get(url_api));
	}	
	getDentistMessages(id: string){
		const url_api = ` http://192.168.0.130:3032/api/messages?filter[where][iddentist]=${id}`;
  	...
  	updateTix(tix :TixInterface, id: string){
		...
		const url_api=` http://192.168.0.130:3032/api/tixes/${id}`;
		return this.http
	...
	getAllDentistsReturn(){
		const url_api = ' http://192.168.0.130:3032/api/dentist?filter[where][status]=activated';
		return (this.dentists = this.http.get(url_api));
	}	
	getAllPatientsReturn(){
		const url_api = ' http://192.168.0.130:3032/api/patient?filter[where][status]=activated';
		return (this.patients = this.http.get(url_api));
	}
	getAllSpecialties(){
		const url_api = ' http://192.168.0.130:3032/api/specialties';
		return (this.specs = this.http.get(url_api));
	}
	getAllTixs(){
		const url_api = ' http://192.168.0.130:3032/api/tixes?filter[where][status]=activated';
		return this.http.get(url_api);
	}
 		getTamano(){
		const url_api = ' http://192.168.0.130:3032/api/tixes?filter[where][status]=activated';
	...
	getTamanoIni(){
		const url_api = ' http://192.168.0.130:3032/api/tixes?filter[where][initload]=activated';
		return (this.tixs = this.http.get(url_api));
	}
...
newdentistrequest(request){
	...
	updateOrder(order :OrderInterface, id: string){
		// let token = this.authService.getToken();
		const url_api=` http://192.168.0.130:3032/api/order/${id}`;
		return this.http
	...
	updateDentist(dentist :DentistInterface, id: string){
		// let token = this.authService.getToken();
		const url_api=` http://192.168.0.130:3032/api/dentist/${id}`;
		return this.http
	...
	updatePatient(patient :PatientInterface, id: string){
		// let token = this.authService.getToken();
		const url_api=` http://192.168.0.130:3032/api/patient/${id}`;
		return this.http
		.put<PatientInterface>(url_api, patient)
		.pipe(map(data => data));
	}
	getOrderByNpedido(npedido: string){
		const url_api = ` http://192.168.0.130:3032/api/order?filter[where][npedido]=${npedido}`;
	...
	getDentistByUserd(userd: string){
		const url_api = ` http://192.168.0.130:3032/api/dentist?filter[where][userd]=${userd}`;
	...
	// 	let indice = userd;
	// 	const url_api = " http://192.168.0.130:3032/api/dentist?filter[where][userd]=d"+indice;
	// 	return this.http.get(url_api);
	// }
...
getPatientByUserd2(userd: string){
		let indice = userd;
		const url_api =  " http://192.168.0.130:3032/api/patient?filter[where][userd]=p"+indice;
		...
		let indice = id;
		const url_api=` http://192.168.0.130:3032/api/dentist/${indice}`;
		this.dentist = this.http.get(url_api);
		return (this.dentist);
	}
