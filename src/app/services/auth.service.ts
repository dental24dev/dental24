...
export class AuthService {
  ...
  constructor(private http: HttpClient) { }
	...
	registerUser(name :string, email: string, password: string,usertype:string, status:string){
		const url_api ='http://192.168.43.103:3032/api/Users';
		return this.http
	...
	loginUser(email:string, password:string):Observable<any>{
		const url_api ='http://192.168.43.103:3032/api/Users/login?include=user';
		return this.http
    ...
    let user_string = localStorage.getItem("currentUser");
	 ...
	 logoutUser(){
	  	let accessToken = localStorage.getItem('accessToken');
		  	const url_api = 'http://192.168.43.103:3032/api/users/logout?access_token=${accessToken}';
